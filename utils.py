...
from thop import profile  # 导入thop模块，该模块提供了一个profile函数用于计算模型的FLOPs和参数量

# 定义一个函数，用于计算模型的参数量和浮点运算次数
def cal_params_flops(model, size):
    # 创建一个随机初始化的输入张量，形状为(1, 3, size, size)
    # 这里的1代表批量大小，3代表图像的通道数，size代表图像的高和宽
    input = torch.randn(1, 3, size, size)
    
    # 使用profile函数计算模型的浮点运算次数和参数量
    # 函数返回两个值：flops代表浮点运算次数，params代表参数量
    flops, params = profile(model, inputs=(input,))
    
    # 打印浮点运算次数，单位是十亿（Giga FLOPs）
    print('flops', flops / 1e9)
    
    # 打印参数量，单位是百万（Mega parameters）
